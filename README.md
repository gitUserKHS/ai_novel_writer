...
- include training metrics and artifact lineage in the autogenerated Hub card when `conarrative_training_metadata.json` exists
- suggest a standardized repo id and next release tag for team-wide naming consistency
- assemble a collaborator onboarding package that pulls shared adapters and writes a ready-to-import runtime preset
...
Every successful `train_qlora.py` run now writes:

- `conarrative_training_metadata.json`

inside the output directory. When you publish that directory, the autogenerated Hub card includes:

- train / eval file lineage
- train / eval example counts
- train / eval metrics
- best checkpoint metadata when available
...
For collaborator onboarding, pull shared roles and emit a runtime preset package in one step:

```powershell
.\.venv\Scripts\python.exe scripts\onboard_hf_collaborator.py --writer-repo-id your-org/conarrative-writer-qwen3-4b-sft-lora --critic-repo-id your-org/conarrative-critic-qwen3-4b-consistency-lora --world-repo-id your-org/conarrative-world-model-qwen3-4b-sft-lora --preset-name team-runtime --save-ui-preset --ui-presets-path configs\workspace_quickstart_mock\ui_presets.json
```

The onboarding package writes:

- `runtime_preset.json`
- `hf_onboarding_manifest.json`
- `ui_runtime_preset_record.json`

and can also save the runtime preset directly into the local UI preset store so collaborators can open the Studio and use it immediately.
